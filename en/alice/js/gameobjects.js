...
var GameObjects = (function() {
  ...
  var lab = $.extend({}, labPrototype, ObjectStorage.load('lab_a_e'));
  ...
  var research = $.extend([], Helpers.loadFile('json/research.json'),
                          ObjectStorage.load('research_a_e'));
  ...
  var workers = $.extend([], Helpers.loadFile('json/workers.json'),
                         ObjectStorage.load('workers_a_e'));
  ...
  var upgradesPrototype = {
    ...
    is_visible: function() {
      return ( !this.used && this.hasReceiver() && lab.money >= this.cost * .7 || this.used);
    },
    is_used: function(){
      return this.used;
    },
  ...
  var upgrades = $.extend([], Helpers.loadFile('json/upgrades.json'),
                         ObjectStorage.load('upgrades_a_e'));
  ...
  var saveAll = function() {
    ObjectStorage.save('lab_a_e', lab);
    ObjectStorage.save('research_a_e', research);
    ObjectStorage.save('workers_a_e', workers);
    ObjectStorage.save('upgrades_a_e', upgrades);
    ObjectStorage.save('achievements_a_e', achievements);
  };
