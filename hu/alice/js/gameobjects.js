...
var GameObjects = (function() {
  ...
  var labPrototype = {
    version: '0.2',
    name: 'Kattints ide és adj egy hangzatos nevet a laborodnak',
    detector: {
      rate: 1
    },
    reputation: 0,
...
/*        this.factor.rateadd = 0.1 * Math.pow ( this.reputation, 1.5) - 0.1 * Math.pow ( this.reputation , 1.45) + this.factor.rate * 0.1 ;*/
        return true;
      }
      return false;
    },
  ...
  var lab = $.extend({}, labPrototype, ObjectStorage.load('lab_a'));
  ...
  var workersPrototype = {
    hired: 0,
    is_visible: function() {
      return this.hired > 0 || lab.money >= this.cost * .5;
    },
    is_available: function() {
  ...
  var upgradesPrototype = {
    ...
    is_visible: function() {
      ...
      return ( !this.used && this.hasReceiver() && lab.money >= this.cost * .5  || this.used);
    },
    is_used: function(){
      return this.used;
    },
  ...
  var saveAll = function() {
    ObjectStorage.save('lab_a', lab);
    ObjectStorage.save('research_a', research);
    ObjectStorage.save('workers_a', workers);
    ObjectStorage.save('upgrades_a', upgrades);
    ObjectStorage.save('achievements_a', achievements);
  };
